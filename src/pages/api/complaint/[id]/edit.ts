...
const ComplaintEditHandler = async (
  req: NextApiRequest,
  res: NextApiResponse
) => {
  ...
  if (req.method === "PUT") {
    try {
      ...
      const complaint = await caller.complaints.update(request);
      logInfo(
      ...
      logError(
        `Error log at EDIT COMPLAINT response : ${JSON.stringify(cause)}`
      );
...
export default ComplaintEditHandler;
