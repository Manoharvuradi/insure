...
const PolicyEditHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === "PUT") {
    try {
      ...
      const complaint = await caller.complaintNotes.update(request);
      ...
      logError(
        `Error log at  EDIT NOTE COMPLAINT  response : ${JSON.stringify(cause)}`
      );
      res.status(500).json({ status: false, message: "Internal server error" });
    }
  } else {
    logError(`Error log at  EDIT NOTE COMPLAINT : method not allowed`);
