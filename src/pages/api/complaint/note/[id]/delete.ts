  ...
  if (req.method == "DELETE") {
    try {
      ...
      if (!id || typeof id !== "string") {
        ...
        const application = await caller.complaintNotes.delete(input);
      ...
      logError(
        `Error log at  DELETE NOTE COMPLAINT response : ${JSON.stringify(
          cause
        )}`
      );
    ...
    logError(`Error log at DELETE NOTE COMPLAINT : method not allowed`);
