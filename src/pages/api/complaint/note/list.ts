...
const PolicyListHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === "GET") {
    try {
      const complaint = await caller.complaintNotes.list();
      ...
      logError(
        `Error log at  LIST NOTE COMPLAINT response : ${JSON.stringify(cause)}`
      );
      res.status(500).json({ status: false, message: "Internal server error" });
    }
  } else {
    logError(`Error log at  LIST NOTE COMPLAINT : method not allowed`);
