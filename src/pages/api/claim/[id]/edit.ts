...
const ClaimEditHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === "PUT") {
    try {
      ...
      const claim = await caller.claim.update(request);
      logInfo(`Success log at EDIT CLAIM response : ${JSON.stringify(claim)}`);
      ...
      logError(`Error log at EDIT CLAIM response : ${JSON.stringify(cause)}`);
      res.status(500).json({ status: false, message: "Internal server error" });
    }
  } else {
    logError(`Error log at EDIT CLAIM : Method not allowed`);
