...
const PolicyEditHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === "PUT") {
    try {
      ...
      const application = await caller.application.updatePayment(request);
      logInfo(
        `Success log at PAYMENT METHOD APPLICATION response : ${JSON.stringify(
          application
        )}`
      );
      ...
      logError(
        `Error log at PAYMENT METHOD APPLICATION response : ${JSON.stringify(
          cause
        )}`
      );
    ...
    logError(`Error log at PAYMENT METHOD APPLICATION : Method not allowed`);
