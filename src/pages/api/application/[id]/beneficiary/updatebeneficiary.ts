...
const ApplicationEditHandler = async (
  ...
  if (req.method === "PUT") {
    try {
      ...
      const application = await caller.application.updateBeneficiary(request);
      logInfo(
        `Success log at BENEFICIARY APPLICATION response : ${JSON.stringify(
          application
        )}`
      );
      ...
      logError(
        `Error log at BENEFICIARY APPLICATION response : ${JSON.stringify(
          cause
        )}`
      );
    ...
    logError(`Error log at BENEFICIARY APPLICATION : Method not allowed`);
