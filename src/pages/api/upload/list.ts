...
const PolicyHolderListHandler = async (
  ...
  if (req.method === "GET") {
    try {
      const policyholder = await caller.uploadLibrary.list();
      ...
      logError(`Error log at LIST UPLOAD response : ${JSON.stringify(cause)}`);
      res.status(500).json({ status: false, message: "Internal server error" });
    }
  } else {
    logError(`Error log at LIST UPLOAD : method not allowed`);
