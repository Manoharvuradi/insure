...
const PolicyEditHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === "PUT") {
    try {
      ...
      const policy = await caller.policy.update(request);
      logInfo(
        `Success log at EDIT POLICY response : ${JSON.stringify(policy)}`
      );
      ...
      logError(`Error log at EDIT POLICY response : ${JSON.stringify(cause)}`);
      res.status(500).json({ status: false, message: "Internal server error" });
    }
  } else {
    logError(`Error log at EDIT POLICY : method not allowed`);
