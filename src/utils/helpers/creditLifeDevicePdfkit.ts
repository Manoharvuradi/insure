...
export const creditLifeDevicePdfkit = async (
  reqData: any,
  packageData: any
) => {
  ...
  const pdfBuffer = await new Promise<string>((resolve) => {
    ...
    doc
      .font("Helvetica")
      .fillColor("black")
      .text("Telkom DEVICE PLAN", 250, doc.page.height - 100);
    doc
      .fillColor("black")
      .text(
    ...
    doc
      ...
      .fontSize(8)
      .text(
        "R " + packageData[0]?.freeCoverBenefitAmount,
        doc.page.width / 2,
        100
      );
    doc
      ...
      .fontSize(8)
      .text(
        "R" + reqData?.policyData?.deviceCreditLife?.freeCoverPremium,
        (doc.page.width * 3) / 4,
        100
      );
    doc
    ...
    doc
      ...
      .fontSize(8)
      .text(
        "R " + reqData?.policyData?.deviceCreditLife?.sumAssured,
        doc.page.width / 2,
        120
      );
    doc
      ...
      .fontSize(8)
      .text(
        "R" + reqData?.policyData?.deviceCreditLife?.additionalPremium,
        (doc.page.width * 3) / 4,
        120
      );
    doc
